...
use crate::{facade, fastforwarder, hooks, shadow};
                    ...
                    Box::new(move |mut core| {
                        handle.block_on(async {
                            core.gba_mut().cpu_mut().set_gpr(
                                4,
                                joyflags.load(std::sync::atomic::Ordering::Relaxed) as i32 | 0xfc00,
                            );
                        });
                    }),
                )
                    ...
                    Box::new(move |core| {
                        handle.block_on(async {
                            ...
                            if !round.has_committed_state() {
                                ...
                                log::info!("primary rng1 state: {:08x}", munger.rng1_state(core));
                                log::info!("primary rng2 state: {:08x}", munger.rng2_state(core));
                                log::info!("battle state committed on {}", current_tick);
                            }

                            if !round
                                .add_tx_and_fastforward(
                                    core,
                                    current_tick + 1, // The tx packet is destined for the next tick.
            ...
            },
            {
                ...
                (
                    ...
                    Box::new(move |core| {
                        ...
                        if ip.local.tick != ip.remote.tick {
                            shadow_state.set_error(anyhow::anyhow!(
                                "shadow read joyflags: local tick != remote tick (in battle tick = {}): {} != {}",
                                current_tick,
                                ip.local.tick,
                                ip.remote.tick
                            ));
                ...
                (
                    ...
                    Box::new(move |mut core| {
                        ...
                        if ip.local.local_tick != ip.remote.local_tick {
                            ff_state.set_error(anyhow::anyhow!(
                                "ff read joyflags: local tick != remote tick (in battle tick = {}): {} != {}",
                                current_tick,
                                ip.local.local_tick,
                        ...
                        if ip.local.local_tick != current_tick {
                            ff_state.set_error(anyhow::anyhow!(
                                "ff read joyflags: input tick != in battle tick: {} != {}",
                                ip.local.local_tick,
                                current_tick,
                            ));
                            return;
            ...
            },
            {
                ...
                (
                    ...
                    Box::new(move |core| {
                        ...
                        if ip.local.local_tick != ip.remote.local_tick {
                            ff_state.set_error(anyhow::anyhow!(
                                "ff copy input data: local tick != remote tick (in battle tick = {}): {} != {}",
                                current_tick,
                                ip.local.local_tick,
                                ip.remote.local_tick
                            ));
                        ...
                        if ip.local.local_tick != current_tick {
                            ff_state.set_error(anyhow::anyhow!(
                                "ff copy input data: input tick != in battle tick: {} != {}",
                                ip.local.local_tick,
                                current_tick,
                            ));
                            return;
